...
class Buffer {
    ...
    write(byte) {
        let result = this.cur_fragment.write(byte);
        if (!result) {
    ...
    get(idx) {
        ...
        while (fi < this.fragment_list.length) {
            let frag = this.fragment_list[fi];
            if (idx < frag.size) {
                return frag.get(idx);
            }
            idx -= frag.size;
    ...
    shrink() {
        let total_buffer = new Uint8Array(this.size());
        let offset = 0;
        for (let i = 0; i < this.fragment_list.length; i++) {
            let frag = this.fragment_list[i];
            if (frag.full()) {
...
var combine_uint8array = function(buffers) {
    ...
    let total_buffer = new Uint8Array(total_size);
    let offset = 0;
    for (var i = 0; i < buffers.length; i++) {
        var buffer = buffers[i];
...
class MobiFile {
    ...
    getUint8() {
        let v = this.view.getUint8(this.offset);
        this.offset += 1;
        return v;
    }

    getUint16() {
        let v = this.view.getUint16(this.offset);
        this.offset += 2;
        return v;
    }

    getUint32() {
        let v = this.view.getUint32(this.offset);
        this.offset += 4;
        return v;
    }

    getStr(size) {
        let v = ab2str(this.buffer.slice(this.offset, this.offset + size));
        this.offset += size;
        return v;
    }
    ...
    get_record_extrasize(data, flags) {
        ...
        for (let i = 15; i > 0; i--) {
            if (flags & (1 << i)) {
                let res = this.buffer_get_varlen(data, pos);
                let size = res[0];
                let l = res[1];
                pos = res[2];
                pos -= size - l;
    ...
    read_text() {
        let text_end = this.palm_header.record_count;
        let buffers = [];
        for (let i = 1; i <= text_end; i++) {
            buffers.push(this.read_text_record(i));
        }
        let all = combine_uint8array(buffers);
        return ab2str(all);
    }

    read_text_record(i) {
        let flags = this.mobi_header.extra_flags;
        let begin = this.reclist[i].offset;
        let end = this.reclist[i+1].offset;

        let data = new Uint8Array(this.buffer.slice(begin, end));
        let ex = this.get_record_extrasize(data, flags);

        data = new Uint8Array(this.buffer.slice(begin, end - ex));
        if (this.palm_header.compression === 2) {
            let buffer = uncompression_lz77(data);
    ...
    read_image(idx) {
        let first_image_idx = this.mobi_header.first_image_idx;
        let begin = this.reclist[first_image_idx + idx].offset;
        let end = this.reclist[first_image_idx + idx + 1].offset;
        let data = new Uint8Array(this.buffer.slice(begin, end));
    ...
    load_record0() {
        ...
        console.log('palm header', this.palm_header);
        this.mobi_header = this.load_mobi_header();
        console.log('mobi_header', this.mobi_header);

    }

    load_record0_header() {
        let p_header = {};
        let first_record = this.reclist[0];
    ...
    render_to(id) {
        this.load();
        let content = this.read_text();
        console.log(this.pages);

        let bookDom = document.getElementById(id);
        let bookDoc = domParser.parseFromString(content, 'text/html');
        bookDom.innerHTML = bookDoc.body.innerHTML;

        let imgDoms = bookDom.getElementsByTagName('img');
    ...
    render_image(imgDoms, i) {
        ...
        let idx = +imgDom.getAttribute('recindex');
        let blob = this.read_image(idx - 1);
